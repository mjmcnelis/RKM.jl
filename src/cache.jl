...
struct UpdateCache{T} <: RKMCache where T <: AbstractFloat
    dy::Matrix{T}
    # dy_LM::Matrix{T}
    y::Vector{T}
    y_tmp::Vector{T}
    f_tmp::Vector{T}
    f::Vector{T}
    # J::Matrix{T}#Union{Matrix{T}, SparseMatrixCSC{T,Int64}}
    y1::Vector{T}
    y2::Vector{T}
    error::Vector{T}
    S::Matrix{T}
...
function UpdateCache(precision::Type{T}, y::Vector{T}, method::ODEMethod,
    ...
    return UpdateCache{precision}(dy, y, y_tmp, f_tmp, f, y1, y2, error, S, S_tmp)#, dS)
end
