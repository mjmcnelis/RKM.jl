...
"""
    nansafe_jacobian(y0::Vector{T}, t0::T1, dy_dt!::Function,
                     ...
                     chunk_size::Int64 = 12) where {T <: AbstractFloat,
                                                    T1 <: AbstractFloat}

Estimates the sparsity pattern of the system Jacobian J = df/dy via forward-mode
...
remove excess matrix elements in the sparsity pattern. For more details
see https://juliadiff.org/ForwardDiff.jl/stable/user/advanced/.

Note: some nonzero matrix elements may be missed if the user function dy_dt!
      handles NaN values in certain edge cases (e.g. if isnan(x) x = 0 end).

Required parameters: `y0`, `t0`, `dy_dt!`

Optional parameters: `p`, `abstract_params`, `chunk_size`
"""
